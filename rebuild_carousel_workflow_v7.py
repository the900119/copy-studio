#!/usr/bin/env python3
"""
Carousel workflow v7:
- Add long_text mode (mode='from_article'): Claude reads the full article,
  auto-extracts topic/hook/method/proof/CTA, then generates 8 VIRAL slides
- Keep existing manual mode for backward compat
"""
...
# Node 2: Build prompt — handles both long_text mode and manual mode
CODE_BUILD_PROMPT = r"""
...
if (mode === 'from_article') {
  // ── Long text mode: let Claude do the full analysis ──
...
【你的任務】
1. 從文章中萃取：最核心的洞察/主題（作為封面Hook）、關鍵方法或觀點（2-3個）、任何數字/結果/社會證明
2. 判斷最適合的封面鉤子類型（殺很大真相/內行人才知道/成功預告/成果搶先看）
...
{"slides":[{"slide_num":1,"role":"封面","headline":"主標題15字以內衝擊感","subheadline":"副標20字以內","body":"內文50-80字第一人稱","photo_query":"english photo keywords for Unsplash 3 words"}]}`;
  ...
  // ── Manual mode: use provided fields ──
...
return [{
  json: {
    prompt: PROMPT,
    topic: topic_for_feishu,
    identity
  }
}];
"""

# Node 4: Parse + Unsplash + Feishu (same as v6)
CODE_PARSE_FETCH = r"""
...
connections = {
    ...
    "Code - Parse & Fetch": {"main": [[{"node": "Respond to Webhook", "type": "main", "index": 0}]]}
}
...
print("Deploying v7 (long_text mode)...")
