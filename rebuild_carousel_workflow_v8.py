#!/usr/bin/env python3
"""
Carousel workflow v8:
- Parallel Unsplash fetches (was sequential, too slow)
- Respond to webhook FIRST, then save Feishu async (avoid timeout)
- Split into: Respond node fires right after Parse&Fetch,
  Feishu save is a separate non-blocking Code node after respond
  (n8n supports this via two outputs / post-respond execution)
"""
...
return [{ json: { prompt: PROMPT, topic: topic_for_feishu, identity } }];
"""

# Parse Anthropic + Parallel Unsplash — NO Feishu here (moved out to avoid timeout)
CODE_PARSE_FETCH = r"""
...
try {
  ...
  if (startIdx === -1 || endIdx === -1) throw new Error('No JSON: ' + cleaned.substring(0, 200));
  const parsed = JSON.parse(cleaned.substring(startIdx, endIdx + 1));
  slides = parsed.slides || [];
  if (slides.length === 0) throw new Error('Empty slides');
} catch (e) {
  return [{ json: { success: false, error: 'Parse error: ' + e.message, rawText: rawText.substring(0, 300), topic, identity } }];
}
...
// PARALLEL Unsplash fetches — much faster than sequential
const photoPromises = slides.map((s, i) => getPhoto.call(this, s.photo_query, i));
const photos = await Promise.all(photoPromises);
const enrichedSlides = slides.map((s, i) => ({ ...s, photo: photos[i] || { url: null, author: 'Unsplash' } }));

return [{ json: { success: true, slides: enrichedSlides, count: enrichedSlides.length, topic, identity } }];
"""

# Feishu save — runs AFTER Respond to Webhook (non-blocking)
CODE_FEISHU_SAVE = r"""
...
if (!data.success) return [{ json: { skipped: true } }];

const { slides, topic, identity } = data;
try {
  ...
  if (!token) return [{ json: { saved: false } }];
  ...
  return [{ json: { saved: false, error: e.message } }];
}
"""
...
nodes = [
    ...
    },
    {
        ...
        "typeVersion": 2,
        "position": [820, 300],
        "parameters": {"jsCode": CODE_PARSE_FETCH}
    },
    {
        "id": "respond-node",
    ...
    {
        "id": "code-feishu-save",
        "name": "Code - Feishu Save",
        "type": "n8n-nodes-base.code",
        "typeVersion": 2,
        "position": [1040, 460],
        "parameters": {"jsCode": CODE_FEISHU_SAVE}
    }
]

connections = {
    "Webhook": {"main": [[{"node": "Code - Build Prompt", "type": "main", "index": 0}]]},
    "Code - Build Prompt": {"main": [[{"node": "HTTP - Anthropic", "type": "main", "index": 0}]]},
    "HTTP - Anthropic": {"main": [[{"node": "Code - Parse & Fetch", "type": "main", "index": 0}]]},
    "Code - Parse & Fetch": {"main": [
        [{"node": "Respond to Webhook", "type": "main", "index": 0}],
        [{"node": "Code - Feishu Save", "type": "main", "index": 0}]
    ]}
}
...
print("Deploying v8 (parallel Unsplash, async Feishu)...")
res = requests.put(f"{N8N_BASE}/api/v1/workflows/{WORKFLOW_ID}", headers=headers, json=payload)
